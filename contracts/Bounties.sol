...
contract Bounties {
  ...
  mapping(uint=>Token) tokenContracts;
  ...
  /// @dev Bounties(): instantiates
  /// @param _tokenAddress the token contract
  function Bounties(address _tokenAddress)
      public
  {
      owner = msg.sender;
      ...
      returns (uint)
  {
      ...
      if (_paysTokens){
        tokenContracts[bounties.length - 1] = Token(_tokenContract);
      }
      ...
      returns (uint)
  {
      ...
      if (_paysTokens){
        require(msg.value == 0);
        tokenContracts[bounties.length] = Token(_tokenContract);
